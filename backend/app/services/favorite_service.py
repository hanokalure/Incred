...
async def add_favorite(user_id: str, place_id: int) -> Dict[str, Any]:
    admin = await get_supabase_client(anon=False)
    try:
        result = await admin.table("favorites").insert({"user_id": user_id, "place_id": place_id}).execute()
...
async def list_favorites(user_id: str) -> List[Dict[str, Any]]:
    admin = await get_supabase_client(anon=False)
    result = await admin.table("favorites").select("id,user_id,place_id,created_at").eq("user_id", user_id).order("created_at", desc=True).execute()
    return result.data or []


async def list_favorite_places(user_id: str) -> List[Dict[str, Any]]:
    """
...
async def delete_favorite(user_id: str, favorite_id: int) -> Dict[str, Any]:
    admin = await get_supabase_client(anon=False)
    result = await admin.table("favorites").delete().eq("id", favorite_id).eq("user_id", user_id).execute()
    if not result or not result.data:
